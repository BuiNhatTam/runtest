...
public class FrmTacGia extends JFrame implements ActionListener {
	 ...
	 * @throws Exception 
	 */
	public FrmTacGia() throws Exception {
		...
		JLabel lblMaTheLoai = new JLabel("Mã Tác Giả");
		...
		lblTenTacGia.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		lblTenTacGia.setBounds(40, 55, 140, 35);
		panel.add(lblTenTacGia);
		
		tfMaTacGia = new JTextField();
		tfMaTacGia.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		tfMaTacGia.setColumns(10);
		tfMaTacGia.setBounds(170, 17, 500, 30);
		panel.add(tfMaTacGia);
		
		tfTacGia = new JTextField();
		tfTacGia.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		tfTacGia.setColumns(10);
		tfTacGia.setBounds(170, 60, 500, 30);
		panel.add(tfTacGia);
		...
		JLabel lblDanhMcSch = new JLabel("Tác Giả");
		...
		table.addMouseListener(new MouseAdapter()
        {
            ...
            public void mouseClicked(MouseEvent e)
            {
                ...
                tfMaTacGia.setText(table.getModel().getValueAt(i, 0).toString());
                tfTacGia.setText(table.getModel().getValueAt(i, 1).toString());
            }
        });
		...
		btnThem.addMouseListener((MouseListener) new MouseAdapter()
        {
            ...
            public void mouseClicked(MouseEvent e)
            {
            	try {
            		...
            		tacGia_dao.themTacGia(tacgia);
            		getDsTheLoai();
            		JOptionPane.showMessageDialog(null, "Thành Công: Thêm tác giả thành công!!");
            		//clean TEXTFIELD
            		clearTextField();
            		
            	} catch (Exception ex) {
					// TODO: handle exception
            		JOptionPane.showMessageDialog(null, "Lỗi khi thêm Tác Giả");
				}
            }
        });
		...
		btnXoa.addMouseListener(new MouseAdapter()
        {
            ...
            public void mouseClicked(MouseEvent e)
            {
            	...
            	if (selectedRow >= 0) {
            		int i = JOptionPane.showConfirmDialog(null, "Xác Nhận Xóa Tác Giả","",JOptionPane.YES_NO_OPTION);
            		if(i==JOptionPane.YES_OPTION)
            		{
            			...
            			if(maTL != "") {
            				tacGia_dao = new TacGia_dao();
            				try {
            					tacGia_dao.xoaTacGiaTheoMa(maTL);
							} catch (Exception e1) {
		...
		btnSua.addMouseListener(new MouseAdapter()
        {
            ...
            public void mouseClicked(MouseEvent e)
            {
                ...
                try {
					tacGia_dao = new TacGia_dao();
					tacGia_dao.updateTacGia(tacgia);
				} catch (Exception e1) {
					e1.printStackTrace();
				}
                try {
	...
	public void outModel(DefaultTableModel model,ArrayList<TacGia> listTacGia)
    {
        Vector row;
        model.setRowCount(0);
